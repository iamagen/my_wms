...
void manager_login()
{
	char name[31], pass[31], pass2[31];	
	
	printf("\n请登录！\n");
	printf("管理员用户名：");
	scanf("%s", name);
	strcpy(pass, getpass("管理员密码："));

	if(is_first_run())
	{
		if(strcmp(name, "admin") != 0 || strcmp(pass, "123456") != 0)
		{
		...
		while(1)
		{
			strcpy(pass, getpass("请重置密码："));
			strcpy(pass2, getpass("再次输入密码："));

			if(strcmp(pass, pass2) == 0) break;
